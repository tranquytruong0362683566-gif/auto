...
Đang tự động gọi nút Đăng xuất trên Extension...`, 'error');
  ...
  async function commentToFacebook(link, comment) {
    ...
    try {
      commentImage = await window.chatGPTApiController?.getActiveCommentImage?.();
    } catch (error) {
      throw new Error(`Không đọc được ảnh bình luận: ${error?.message || error}`);
    }

    S.setBridgeStatus(
      commentImage
        ? 'Đang mở tab Facebook ẩn, tải ảnh lên trước rồi gửi nội dung bình luận...'
        : 'Đang mở tab Facebook ẩn, chờ tải trang và gửi bình luận...',
      'warn'
    );
    ...
    const response = await API.sendBridge(
      ['COMMENT_IN_FB_TAB', 'COMMENT_FB_POST', 'COMMENT_FACEBOOK_POST', 'commentFbPost', 'commentFacebookPost', 'COMMENT_POST', 'COMMENT_CURRENT_TAB'],
      {
        ...
        text: comment,
        commentText: comment,
        imageDataUrl: commentImage?.dataUrl || '',
        imageName: commentImage?.name || '',
        imageType: commentImage?.type || '',
        imageSize: Number(commentImage?.size || 0),
        imageRequired: Boolean(commentImage?.required),
        imageWaitMs: 20000,
    ...
    if (commentImage?.required && responseData?.imageAttached !== true) {
      throw new Error(responseData?.imageAttachError || 'Facebook chưa xác nhận ảnh bình luận đã được đính kèm.');
    }
    if (commentImage && responseData?.imageAttached === true) {
      S.setBridgeStatus('Đã gửi ảnh và nội dung bình luận xong.', 'ok');
    } else if (commentImage) {
      S.setBridgeStatus('Không gắn được ảnh tùy chọn; đã gửi nội dung chữ.', 'warn');
    } else {
  ...
  async function commentCurrentTab() {
    ...
    if (isNextCommentResult(comment)) {
      const skippedLink = S.getPostLinks()[0] || '';
      await closeActiveReadTabIfAny();
      if (skippedLink) {
        S.saveRemovedLink(skippedLink);
        S.setPostLinks(S.getPostLinks().filter(item => S.normalizeUrl(item) !== S.normalizeUrl(skippedLink)));
      }
      S.setBridgeStatus('Kết quả là (next): đã chuyển link sang Link đã loại bỏ và không gửi bình luận.', 'warn');
      return { ok: true, skipped: true, reason: 'next' };
    }
    const link = S.getPostLinks()[0] || '';
      ...
      for (let index = 0; index < queue.length; index += 1) {
        ...
        try {
          ...
          if (isNextCommentResult(comment) || controller.isNextResult?.(comment)) {
            const campaignLabel = controller.getActiveCampaign?.()?.context?.category || 'chiến dịch hiện tại';
            S.setBridgeStatus(`AI xác định link ${index + 1}/${queue.length} không phù hợp với ${campaignLabel}, đã chuyển ngay sang bài tiếp theo.`, 'warn');
            await closeActiveReadTabIfAny();
